...
\end{equation}
validating the lumped-sum approximation.
...
Using the complete analytic solution yielded a $t_{final}=1992.525\mathrm{s}$:
\begin{gather*}
    ...
    t_{final}=8.68\times 229.5\mathrm{s}=\boxed{1992.525\mathrm{s}}
\end{gather*}
Figure~\ref{fig:a1-full} showcases the analytic solution across $0\leq t\leq 1992.525\mathrm{s}$, while Figure~\ref{fig:a1-zoom95} focuses on behavior near $t\approx95$s. Based on the low Biot number suggesting validity of the lumped sum approach, and the temperature at $T(t=95\mathrm{s})\approx64^\circ$C closely matching the known observation ($T_{surf}(95\mathrm{s})=64^\circ$C), the analytic solution appears to be a reasonably accurate description of the system, with temperature gradually approaching 25$^\circ$C in exponential decay.
\begin{figure}
    \centering
    \includegraphics[width=\figwidth]{fig/A1_full.png}
    \caption{The lumped sum analytical solution of $T(t)$ across times $0\leq t\leq 1992.525\mathrm{s}$.}
    \label{fig:a1-full}
\end{figure}
\begin{figure}
    \centering\includegraphics[width=\figwidth]{fig/A1_zoom95.png}
    \caption{The lumped sum analytical solution of $T(t)$, focused on $t=95$s.}
    \label{fig:a1-zoom95}
\end{figure}
...
In Figure~\ref{fig:b1-dt}, such numerical solutions without discretization in space (having only a single node, akin to the lumped-sum model), are shown for different discretizations in time, with a timestep $\Delta t$ ranging from the minimum stable value of 172.112s to a value of 9.963 seconds (yielding 200 timesteps).
\begin{figure}
    \centering\includegraphics[width=\figwidth]{fig/B1_dt.png}
    \caption{A single-node explicit finite difference solution for $T(t)$, evaluated for $\Delta t\in\left\{\Delta t_{max}, \frac{t_{final}}{20}, \frac{t_{final}}{200}\right\}$, compared to the lumped sum analytical solution. }    
    \label{fig:b1-dt}
\end{figure}
...
Based on the low Biot number $B_i=0.00249<0.1$ suggesting validity of the lumped sum approach, and the analytical temperature at $T(t=95\mathrm{s})\approx64^\circ$C closely matching the known observation ($T_{surf}(95\mathrm{s})=64^\circ$C) in Figure~\ref{fig:a1-zoom95}, the analytic solution displayed in Figure~\ref{fig:a1-full} appears to be a reasonably accurate description of the system, with temperature gradually approaching 25$^\circ$C in exponential decay.\\
...
Figure~\ref{fig:b1-k} shows the importance of determining whether a lumped-sum approach is valid, and the power of finite difference scheme.
While for the given system, which has a Biot number of $0.00249$, the lumped sum approach is demonstrably valid, with a nearly imperceptible range of temperatures from the center to the edge, the comparable system with less conductivity quickly develops a very substantial gradient in temperature, eventually taking longer to approach fluid temperature as heat struggles to move from the interior to the surface.
...
An analytical model of the system's transient behavior was successfully designed and used to estimate a convection coefficient $h=49.956\mathrm{\frac{W}{m^2\cdot K}}$.
